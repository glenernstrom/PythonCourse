...
if __name__ == "__main__":
    selection = input(
        "Do you wish to look up by element name or by atomic number? e/a: "
    )
    ...
    if selection == "e":
        ...
        if element_name in ELEMENTS:
            print(
                f"The atomic number of {element_name} is"
                f" {ELEMENTS.index(element_name)}."
            )
        else:
            print("Error. Element not found.")
    elif selection == "a":
        atomic_number = int(input("Enter the atomic number: "))
        if atomic_number in range(1, len(ELEMENTS)):
            print(
                f"The element with atomic number {atomic_number} "
